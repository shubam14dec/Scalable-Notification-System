...
/**
 * Per-agent knowledge sources (managed agents): list with status dots, add
 * pasted text or a URL, re-index, delete. Polls every 3s while any source is
 * still pending/indexing, then goes quiet.
 */
function KnowledgeModal({ agent, onClose }: { agent: Agent; onClose: () => void }) {
  const queryClient = useQueryClient();
  ...
  return (
    <Modal open onClose={onClose} title={`Knowledge — ${agent.identifier}`}>
      <p className="mb-4 text-[12px] text-t3">
...
// ---------------------------------------------------------------------------
// Long-term memory (Phase 24 D11) — the durable facts an agent knows about one
// customer. Addressed by the customer's external id (no subscriber-list
// endpoint exists — operators type the id they know).
// ---------------------------------------------------------------------------
...
/**
 * Per-agent, per-customer memory admin: enter a customer id, load their
 * profile, then inline-edit / delete facts or add new ones. A full profile
...
function MemoryModal({ agent, onClose }: { agent: Agent; onClose: () => void }) {
  ...
  return (
    <Modal open onClose={onClose} title={`Memory — ${agent.identifier}`}>
      <p className="mb-4 text-[12px] text-t3">
          ...
          <div className="mt-4 flex items-center justify-between">
            ...
            {memories.length > 0 && (
              <Button
                variant="danger"
                onClick={() => {
                  if (window.confirm(`Delete ALL remembered facts for "${subId}"?`)) del.mutate(undefined);
                }}
              >
                Delete all
              </Button>
          ...
          {cap && (
            <div className="mt-3 rounded-md border border-bd bg-elevated p-3">
              <p className="text-[12px] text-err">
                This profile is full{cap.limits?.maxKeys ? ` (max ${cap.limits.maxKeys} facts)` : ''}.
                Overwrite one of the existing keys, or delete a fact first.
              </p>
...
export default function AgentsPage() {
  ...
  const [knowledgeFor, setKnowledgeFor] = useState<Agent | null>(null);
  const [memoryFor, setMemoryFor] = useState<Agent | null>(null);
              ...
              {data.agents.map((a) => (
                <tr key={a.identifier} className="transition-colors hover:bg-elevated">
                  ...
                  <td className={`${td} text-right whitespace-nowrap`}>
                    ...
                    {a.runtime === 'managed' && (
                      <Button variant="ghost" onClick={() => setKnowledgeFor(a)}>
                        Knowledge
                      </Button>
                    )}
                    {a.runtime === 'managed' && (
                      <Button variant="ghost" onClick={() => setMemoryFor(a)}>
                        Memory
                      </Button>
                    )}
...
http.createServer(createHandler(support, {
  signingSecret: process.env.ASYNCIFY_AGENT_SECRET,
      ...
      {knowledgeFor && (
        <KnowledgeModal agent={knowledgeFor} onClose={() => setKnowledgeFor(null)} />
      )}

      {memoryFor && <MemoryModal agent={memoryFor} onClose={() => setMemoryFor(null)} />}
